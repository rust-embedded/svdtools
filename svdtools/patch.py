...
def process_peripheral_regs_array(ptag, rspec, rmod):
    ...
    dimIndex = (
        "{0}-{0}".format(registers[0][1])
        if dim == 1
        else ",".join([r[1] for r in registers])
    )
    ...
    if not (
        check_offsets(offsets, dimIncrement) and check_bitmasks(bitmasks, bitmasks[0])
    ):
        raise SvdPatchError(
...
def process_register_field(pname, rtag, fspec, field):
    ...
    if isinstance(field, dict):
        ...
        for usage in (u for u in usages if u in field):
            process_field_enum(
                pname, rtag, fspec, field[usage], usage=usage.replace("_", "")
            )
    ...
    for ptag in iter_peripherals(svd, pspec):
        ...
        for rspec in peripheral:
            if not rspec.startswith("_"):
                ...
                process_peripheral_register(ptag, rspec, register, update_fields)
...
def process_device(svd, device, update_fields=True):
    ...
    for periphspec in device:
        if not periphspec.startswith("_"):
            ...
            process_peripheral(svd, periphspec, device[periphspec], update_fields)
